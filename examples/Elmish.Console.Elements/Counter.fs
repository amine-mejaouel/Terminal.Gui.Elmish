...
type Model = { Counter: int; IsSpinning: bool }
...
let init () : Model * Cmd<Msg> =
  let model = { Counter = 0; IsSpinning = false }
  model, Cmd.none
...
let update (msg: Msg) (model: Model) =
  ...
  | StartSpin -> { model with IsSpinning = true }, Commands.startSpinning true
  | StopSpin -> { model with IsSpinning = false }, Cmd.none
  | Spinned -> model, Commands.startSpinning model.IsSpinning

...
let incrementCmd () =
  Cmd.OfFunc.perform
    (fun () ->
      let result =
        Dialogs.messageBox 60 10 "Increment" "How many do you want to Increment?" [ "+10"; "+5"; "+1"; "None" ]
...
let decrementCmd () =
  Cmd.OfFunc.perform
    (fun () ->
      let result =
        Dialogs.errorBox 60 10 "Decrement" "How many do you want to Decrement?" [ "-10"; "-5"; "-1"; "None" ]
...
let showWizardCmd =
  fun dispatch ->
    ...
    let lbl =
      new Label(Text = "Enter Number to Increase the Counter:", Width = Dim.Auto())

    secondStep.Add(lbl) |> ignore

    let number =
      new TextField(X = Pos.Right(lbl) + Pos.Absolute(1), Width = Dim.Fill() - Dim.Fill(1))

    secondStep.Add(number) |> ignore

    wizard.Finished.Add(fun ev ->
      match System.Int32.TryParse(number.Text.ToString()) with
      | false, _ ->
        MessageBox.Query("Wizard", $"Error. '{number.Text}' is not a number", "Ok")
        |> ignore

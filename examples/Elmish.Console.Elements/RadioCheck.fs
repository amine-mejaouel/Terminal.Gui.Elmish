...
let update (msg: Msg) (model: Model) =
  match msg with
  | ChangeVoteResult vr -> { model with VoteResult = vr }, Cmd.none
  | ChangeHappy b -> { model with IsHappy = b }, Cmd.none
...
let view (model: Model) (dispatch: Msg -> unit) = [
  ...
  View.radioGroup [
    ...
    radioGroupProps.selectedItem (
      model.VoteResultItems
      |> List.findIndex (fun (i, _) -> i = model.VoteResult)
    )
    radioGroupProps.radioLabels (model.VoteResultItems |> List.map snd)
    radioGroupProps.selectedItemChanged (fun r ->
      let v =
        fst model.VoteResultItems.[r.SelectedItem]
  ...
  View.checkBox [
    ...
    checkBoxProps.checkedStateChanging (fun b ->
      dispatch
      <| ChangeHappy(b.NewValue = CheckState.Checked)
    )
  ]
  ...
  View.label [
    ...
    | true -> labelProps.text "You are Happy!"
    | false -> labelProps.text "you are not Happy!"
  ]
]

...
let openFileDialogCmd () =
  Cmd.OfAsync.perform
    (fun () ->
      async {
        let file =
          Dialogs.openFileDialog "Open TextFile"
        ...
        | Some f ->
          try
            let! content =
              System.IO.File.ReadAllTextAsync(f)
              |> Async.AwaitTask
...
let update (msg: Msg) (model: Model) =
  ...
  | OpenFileDialog -> model, openFileDialogCmd ()
  | FileSelected fileContent -> { model with Text = fileContent }, Cmd.none

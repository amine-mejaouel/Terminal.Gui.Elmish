...
type PropertyMetadata =
  ...
  member x.FSharpTypeName = getFSharpTypeName x.PropertyInfo.PropertyType

type EventMetadata = { PKey: string; EventInfo: EventInfo }
...
type ViewMetadata =
  ...
  member x.HasNoEventsOrProperties = (x.Properties.Length = 0 && x.Events.Length = 0)
...
module ViewMetadata =
  ...
  let private readWriteProperties (viewType: Type) =
    viewType.GetProperties(BindingFlags.Public ||| BindingFlags.Instance ||| BindingFlags.DeclaredOnly)
    |> Array.filter (fun p ->
      p.CanRead
      && p.GetMethod.IsPublic
      && p.CanWrite
      && p.SetMethod.IsPublic
      && (not <| hasInitOnlySetter p))
    |> Array.sortBy _.Name

  let private events (viewType: Type) =
    viewType.GetEvents(BindingFlags.Public ||| BindingFlags.Instance ||| BindingFlags.DeclaredOnly)
  ...
  let create (viewType: Type) =
    ...
    let view_Typed_SubElementsProps =
      props
      |> Array.filter (fun p -> p.PropertyType.IsAssignableTo typeof<Terminal.Gui.ViewBase.View>)
      |> Array.sortBy _.Name
    ...
    let excludedProps =
      [| nameof (Unchecked.defaultof<View>.X); nameof (Unchecked.defaultof<View>.Y) |]

    { Type = viewType
      Properties =
        props
        |> Array.map toPropertyMetadata
        |> Array.filter (fun p -> not (Array.contains p.PKey excludedProps))

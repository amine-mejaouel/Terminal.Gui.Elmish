...
let getPropertiesNotInherited (t: Type) =
    ...
    |> Array.filter (fun p ->
        let setMethodReturnParameterModifiers =
            p.SetMethod.ReturnParameter.GetRequiredCustomModifiers()

        setMethodReturnParameterModifiers
        |> Seq.exists (fun t -> t = (typeof<System.Runtime.CompilerServices.IsExternalInit>))
        |> not)
...
let getAllNamespaces (types: Type array) =
    types |> Array.map (fun t -> t.Namespace) |> Array.distinct
...
let rec mapTypeAndSubTypeProperties (t: Type) (subtypes: Type array) =
    ...
    | _ when t.IsArray && t.GetElementType().IsSubclassOf(typeof<View>) -> $"{t.GetElementType().Name} list"
    | _ when t.IsArray && t.GetElementType().IsSubclassOf(typeof<MenuItem>) -> $"{t.GetElementType().Name} list"
    | _ when t.IsArray -> $"{t.GetElementType().Name} list"
...
let printPropertiesProps
    ...
    (properties:
        {| Name: string
           CamelName: string
           Type: string
           OrigType: string |} array)
    =
    properties
    |> Array.iter (fun e ->
        printfn
            $"    static member inline {e.CamelName} (value:{e.Type}) = Interop.mkprop \"{toCamelCase t.Name}.{e.CamelName}\" value")

let printEventsProps
    (printfn: string -> unit)
    (t: Type)
    (events:
        {| Name: string
           CamelName: string
           Type: string
           MapFunction: string |} array)
    =
    events
    |> Array.iter (fun e ->
        printfn
            $"    static member inline {e.CamelName} (handler:{e.Type}) = Interop.mkprop \"{toCamelCase t.Name}.{e.CamelName}\" handler")


let printViewProps (printfn: string -> unit) (view: Type) =
    let propsProps =
        if view.Name = "View" then
            getProperties view |> getPropertyProps
        else
            getPropertiesNotInherited view |> getPropertyProps

    let eventProps =
        if view.Name = "View" then
            getEvents view |> getEventProps
        else
            getEventsNotInherited view |> getEventProps
    ...
    if propsProps.Length + eventProps.Length > 0 then
        ...
        // Header View
        let typeName =
            (if view.Name = "View" then "prop" else toCamelCase view.Name).Replace("`1", "<'a>")

        let typeName =
            if typeName = "treeView<'a>" then
...
type {typeName} =
    ...
    static member inline ref (reference:View->unit) = Interop.mkprop "ref" reference
        ...
        if typeName = "prop" then
            printfn "    // Properties"
            propsProps |> printPropertiesProps printfn view
            printfn "    // Events"
            eventProps |> printEventsProps printfn view
...
let printNamepaces (printfn: string -> unit) =
    ...
    namespaces
    |> Array.filter (fun ns -> String.IsNullOrWhiteSpace ns |> not)
    |> Array.iter (fun ns -> printfn $"open {ns}")
...
let printElementSetProps (printfn: string -> unit) (t: Type) =
    let props =
        if t.Name = "View" then
            ...
            getPropertiesNotInherited t

    let events =
        if t.Name = "View" then
            getEvents t
        else
            getEventsNotInherited t
    ...
    if props.Length + events.Length = 0 then
        ...
        if props.Length > 0 then
            ...
            |> Array.iter (fun e ->
                ...
                | "List`1" ->
                    printfn
                        $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- v.ToList())"""
                | "UInt32" ->
                    printfn
                        $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- (v |> uint32))"""
                | "IListDataSource" ->
                    printfn
                        $"""        props |> Interop.getValue<string list> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.SetSource (ObservableCollection(v)))"""
                | "View" ->
                    printfn
                        $"""
        props
        ...
        |> Option.iter (fun v ->
            ...
            element.View <- v.element
                ...
                | "MenuBarItem[]" ->
                    printfn
                        $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- v |> List.toArray)"""
                | _ ->
                    printfn
                        $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- v {(if e.Type.Contains(" option") then
                                                                                                                                                                  " |> Option.toNullable"
                                                                                                                                                              else
                                                                                                                                                                  "")})""")
        ...
        if events.Length > 0 then
            ...
            |> Array.iter (fun e ->
                if (e.Type = "unit->unit") then
                    printfn
                        $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun v -> Interop.setEventHandler <@ element.{e.Name} @> (fun _ -> v()) element)"
                else
                    printfn
                        $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun v -> Interop.setEventHandler <@ element.{e.Name} @> {e.MapFunction} element)")
    ...
    if props.Length + events.Length = 0 then
        ...
        if props.Length > 0 then
            ...
            |> Array.iter (fun e ->
                ...
                | "IListDataSource" ->
                    printfn
                        $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun _ -> element.SetSource Unchecked.defaultof<_>)"
                | _ ->
                    printfn
                        $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun _ -> element.{e.Name} <- Unchecked.defaultof<_>)")
        ...
        if events.Length > 0 then
            ...
            |> Array.iter (fun e ->
                printfn
                    $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun _ -> Interop.removeEventHandler <@ element.{e.Name} @> element)")
...
let printElement (printfn: string -> unit) (t: Type) =
    let name, isGeneric, onlyRef =
        match t.Name with
        | "TreeView`1" -> t.Name.Replace("`1", ""), true, true
        | x when x.Contains("`1") -> t.Name.Replace("`1", ""), true, false
...
if true then
    let file =
        Path.Combine(__SOURCE_DIRECTORY__, @"..\..\Terminal.Gui.Elmish\Elements.fs")
    // empty file
    File.WriteAllText(file, "")
    let st = new StreamWriter(file)
...
let printViewType (printfn: string -> unit) =
    ...
    |> Array.map (fun e ->
        match e.Name with
        | x when x.Contains("`1") -> e.Name.Replace("`1", ""), true
        | _ -> e.Name, false)
    |> Array.iter (fun (name, isGeneric) ->
        ...
        printfn
            $"""    static member inline {toCamelCase name}{notStruct} (props:IProperty list) = {name}Element{generic}(props) :> TerminalElement"""

        printfn
            $"""    static member inline {toCamelCase name}{notStruct} (children:TerminalElement list) =
        let props = [ prop.children children ]
        {name}Element{generic}(props) :> TerminalElement"""

        printfn
            $"""    static member inline {toCamelCase name}{notStruct} (x:int, y:int, title: string) =
        let props = [

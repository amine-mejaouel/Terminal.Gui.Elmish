...
let rec mapTypeAndSubTypeProperties (t: Type) (subtypes: Type array) =
  ...
  | _ when
    t.IsArray
    && t.GetElementType().IsSubclassOf(typeof<View>)
    ->
    $"{t.GetElementType().Name} list"
  | _ when
    t.IsArray
    && t.GetElementType().IsSubclassOf(typeof<MenuItem>)
    ->
    $"{t.GetElementType().Name} list"
...
let getEventProps (events: (string * Type * Type array) array) =
  ...
  |> Array.map (fun (name, typ, subtypes) ->
    let t, mapFunction =
      mapTypeAndSubTypeEvents typ subtypes
...
let printPropertiesProps
  ...
  =
  properties
  |> Array.iter (fun e -> printfn $"    static member inline {e.CamelName} (value:{e.Type}) = Interop.mkprop \"{toCamelCase t.Name}.{e.CamelName}\" value")
...
let printEventsProps
  ...
  =
  events
  |> Array.iter (fun e -> printfn $"    static member inline {e.CamelName} (handler:{e.Type}) = Interop.mkprop \"{toCamelCase t.Name}.{e.CamelName}\" handler")
...
let printNamepaces (printfn: string -> unit) =
  let namespaces =
    getAllNamespaces inheritedTypes
  // opens
  printfn "open System"
...
if true then
  let file =
    Path.Combine(__SOURCE_DIRECTORY__, @"..\..\Terminal.Gui.Elmish\Props.fs")
  // empty file
  ...
  if props.Length + events.Length = 0 then
    ...
    if props.Length > 0 then
      ...
      |> Array.iter (fun e ->
        ...
        | "IList`1"
        | "List`1" -> printfn $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- v.ToList())"""
        | "UInt32" -> printfn $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- (v |> uint32))"""
        | "IListDataSource" -> printfn $"""        props |> Interop.getValue<string list> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.SetSource (ObservableCollection(v)))"""
        | "View" ->
          printfn
            $"""
        props
        ...
        | "String[]"
        | "MenuBarItem[]" -> printfn $"""        props |> Interop.getValue<{e.Type}> "{toCamelCase t.Name}.{e.CamelName}" |> Option.iter (fun v -> element.{e.Name} <- v |> List.toArray)"""
        | _ ->
          printfn
...
let printElementRemoveProps (printfn: string -> unit) (t: Type) =
  ...
  if props.Length + events.Length = 0 then
    ...
    if props.Length > 0 then
      ...
      |> Array.iter (fun e ->
        match e.OrigType with
        | "IListDataSource" -> printfn $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun _ -> element.SetSource Unchecked.defaultof<_>)"
        | _ -> printfn $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun _ -> element.{e.Name} <- Unchecked.defaultof<_>)"
      )
    ...
    if events.Length > 0 then
      ...
      |> getEventProps
      |> Array.sortBy (fun e -> e.Name)
      |> Array.iter (fun e -> printfn $"        props |> Interop.getValue<{e.Type}> \"{toCamelCase t.Name}.{e.CamelName}\" |> Option.iter (fun _ -> Interop.removeEventHandler <@ element.{e.Name} @> element)")
...
let printElement (printfn: string -> unit) (t: Type) =
  ...
  printfn
    $"""// {name}{(if isGeneric then
                     ($"<'a{onlyRefNotation}>")
                   else
                     "")}"""

  printfn
    $"""type {name}Element{(if isGeneric then
                              ($"<'a{onlyRefNotation}>")
                            else
                              "")}(props:IProperty list) ="""

  let name =
    $"""{name}{(if isGeneric then "<'a>" else "")}"""
...
if true then
  let file =
    Path.Combine(__SOURCE_DIRECTORY__, @"..\..\Terminal.Gui.Elmish\Views.fs")
  // empty file
  File.WriteAllText(file, "")
  let st = new StreamWriter(file)
